 ...
 */
public class ExpectationCalculatorThread<V> implements Runnable {
    ...
    public ExpectationCalculatorThread(HashSet<V> valueSet,  int n) {
    ...
    public void run() {
        ...
        double[] unNormalizedLikelihoods = new double[NUM_CLUSTERS];
        ...
        for (int c = 0; c < NUM_CLUSTERS; c++) {
            ...
            if (logLikelihoods[c] - maxLogLikelihood < -UNDERFLOW_CUTOFF)
                unNormalizedLikelihoods[c] = 0;
            else {
                unNormalizedLikelihoods[c] = Math.exp(logLikelihoods[c] - maxLogLikelihood);
                normalizationConstant += unNormalizedLikelihoods[c];
            }
        }

        for (int c = 0; c < NUM_CLUSTERS; c++) {
            RESPONSIBILITY_TABLE[c][n] = unNormalizedLikelihoods[c] / normalizationConstant;
        }
